...
class HiveMetastore:
    ...
    def write_table(
        ...
        """
        logger.info(f"Writing entities in table {table_name}")
    ...
    def extract_metastore(self):
        ...
        self.ms_database_params = self.read_table(
            connection=self.connection, table_name="DATABASE_PARAMS"
        )
        self.ms_tbls = self.read_table(connection=self.connection, table_name="TBLS")
        self.ms_table_params = self.read_table(
            connection=self.connection, table_name="TABLE_PARAMS"
        )
        self.ms_columns = self.read_table(
            connection=self.connection, table_name="COLUMNS_V2"
        )
        self.ms_bucketing_cols = self.read_table(
            connection=self.connection, table_name="BUCKETING_COLS"
        )
        self.ms_sds = self.read_table(connection=self.connection, table_name="SDS")
        self.ms_sd_params = self.read_table(
            connection=self.connection, table_name="SD_PARAMS"
        )
        self.ms_serdes = self.read_table(
            connection=self.connection, table_name="SERDES"
        )
        self.ms_serde_params = self.read_table(
            connection=self.connection, table_name="SERDE_PARAMS"
        )
        self.ms_skewed_col_names = self.read_table(
            connection=self.connection, table_name="SKEWED_COL_NAMES"
        )
        self.ms_skewed_string_list = self.read_table(
            connection=self.connection, table_name="SKEWED_STRING_LIST"
        )
        ...
        self.ms_sort_cols = self.read_table(
            connection=self.connection, table_name="SORT_COLS"
        )
        self.ms_partitions = self.read_table(
            connection=self.connection, table_name="PARTITIONS"
        )
        self.ms_partition_params = self.read_table(
            connection=self.connection, table_name="PARTITION_PARAMS"
        )
        self.ms_partition_keys = self.read_table(
            connection=self.connection, table_name="PARTITION_KEYS"
        )
        self.ms_partition_key_vals = self.read_table(
            connection=self.connection, table_name="PARTITION_KEY_VALS"
        )
    ...
    def write_tables_to_metastore(self):
        ...
        :return:
        """
        logger.info("Databases to be inserted")
        self.ms_dbs.show()
