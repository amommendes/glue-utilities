...
class HiveMetastoreTransformer:
    ...
    def kv_pair_to_map(self, df, id_col, key, value, map_col_name):
        ...
        map_type = MapType(
            keyType=df.get_schema_type(key), valueType=df.get_schema_type(value)
        )
        ...
        return self.sql_context.createDataFrame(
            ...
            .map(lambda id_name, dictionary: (id_name, remove_none_key(dictionary))),
            output_schema,
        )
    ...
    def transform_df_with_idx(
        self, df, id_col, idx, payloads_column_name, payload_type, payload_func
    ):
        """
        ...
        rdd_result = (
            ...
            .map(
                lambda id_column, list_with_idx: (
                    id_column,
                    sorted(list_with_idx, key=lambda t: t[0]),
                )
            )
            .map(
                lambda id_column, list_with_idx: (
                    id_column,
                    [payload for index, payload in list_with_idx],
                )
            )
        )
        ...
        schema = StructType(
            [
                ...
                StructField(
                    name=payloads_column_name,
                    dataType=ArrayType(elementType=payload_type),
                ),
            ]
        )
    ...
    def modify_column_by_udf(df, udf, column_to_modify, new_column_name=None):
        ...
        return df.select(
            *[
                udf(column).alias(new_column_name)
    ...
    def s3a_or_s3n_to_s3_in_location(df, location_col_name):
        ...
        udf = UserDefinedFunction(
            ...
            if location is None
            else re.sub(r"^s3[a|n]:\/\/", "s3://", location),
            StringType(),
        )
        return HiveMetastoreTransformer.modify_column_by_udf(
            df=df, udf=udf, column_to_modify=location_col_name
        )
    ...
    def add_prefix_to_column(df, column_to_modify, prefix):
        ...
        return HiveMetastoreTransformer.modify_column_by_udf(
            df=df, udf=udf, column_to_modify=column_to_modify
        )
    ...
    def utc_timestamp_to_iso8601_time(df, date_col_name, new_date_col_name):
        ...
        def convert_time(timestamp):
            ...
            return datetime.fromtimestamp(
                timestamp=float(timestamp), tz=UTC()
            ).strftime("%b %d, %Y %I:%M:%S %p")

        udf_time_int_to_date = UserDefinedFunction(convert_time, StringType())
        return HiveMetastoreTransformer.modify_column_by_udf(
            df, udf_time_int_to_date, date_col_name, new_date_col_name
        )
    ...
    def fill_none_with_empty_list(df, column):
        ...
        return HiveMetastoreTransformer.modify_column_by_udf(
            df=df,
            udf=UserDefinedFunction(
                lambda lst: [] if lst is None else lst, get_schema_type(df, column)
            ),
    ...
    def join_dbs_tbls(ms_dbs, ms_tbls):
        return ms_dbs.select("DB_ID", "NAME").join(
            other=ms_tbls, on="DB_ID", how="inner"
        )

    def transform_skewed_values_and_loc_map(
        self, ms_skewed_string_list_values, ms_skewed_col_value_loc_map
    ):
        # columns: (STRING_LIST_ID:BigInt, skewedColumnValuesList:List[String])
        skewed_values_list = self.transform_ms_skewed_string_list_values(
        ...
        skewed_value_str_with_loc = ms_skewed_col_value_loc_map.join(
            ...
            on=[
                ...
                == skewed_value_str["STRING_LIST_ID"]
            ],
            how="inner",
        )
            ...
            schema=StructType(
                [
                    ...
                    StructField(
                        ...
                        dataType=ArrayType(elementType=StringType()),
                    ),
                ]
            ),
        )
    ...
    def transform_skewed_info(
        ...
        ) = self.transform_skewed_values_and_loc_map(
        ...
        skewed_info = skewed_column_names.join(
    ...
    def transform_param_value(self, df):
        udf_escape_chars = UserDefinedFunction(
            HiveMetastoreTransformer.udf_escape_chars, StringType()
        )
    ...
    def transform_ms_serde_info(self, ms_serdes, ms_serde_params):
        ...
        serde_with_params = self.join_with_params(
            df=ms_serdes, df_params=escaped_serde_params, id_col="SERDE_ID"
        )
    ...
    def transform_storage_descriptors(
        ...
        serde_info = self.transform_ms_serde_info(
            ms_serdes=ms_serdes, ms_serde_params=ms_serde_params
        )
        ...
        storage_descriptors_s3_location_fixed = (
            HiveMetastoreTransformer.s3a_or_s3n_to_s3_in_location(
                storage_descriptors_joined, "LOCATION"
            )
        )
        storage_descriptors_renamed = (
            storage_descriptors_s3_location_fixed.rename_columns(
                rename_tuples=[
        ...
        storage_descriptors_with_empty_sorted_cols = (
            HiveMetastoreTransformer.fill_none_with_empty_list(
                storage_descriptors_renamed, "sortColumns"
            )
        )
        storage_descriptors_final = (
            storage_descriptors_with_empty_sorted_cols.drop_columns(
                ["SERDE_ID", "CD_ID"]
            )
        )
    ...
    def transform_tables(
        self, db_tbl_joined, ms_table_params, storage_descriptors, ms_partition_keys
    ):
        ...
        tbls_with_params = self.join_with_params(
            df=tbls_date_transformed,
            df_params=self.transform_param_value(ms_table_params),
            id_col="TBL_ID",
        )
        ...
        tbls_dropped_cols = tbls_renamed.drop_columns(
            ["DB_ID", "TBL_ID", "SD_ID", "LINK_TARGET_ID"]
        )
        tbls_drop_invalid = tbls_dropped_cols.na.drop(
            how="any", subset=["name", "database"]
        )
    ...
    def transform_partitions(
        ...
        parts_with_db_tbl = parts_date_transformed.join(
            other=db_tbl_names, on="TBL_ID", how="inner"
        )
        parts_with_params = self.join_with_params(
            df=parts_with_db_tbl,
        ...
        parts_with_values = parts_with_sd.join(
            other=part_values, on="PART_ID", how="left_outer"
        )
        parts_renamed = rename_columns(
            df=parts_with_values,
            rename_tuples=[
                ("CREATE_TIME", "createTime"),
                ("LAST_ACCESS_TIME", "lastAccessTime"),
            ],
        ...
        parts_drop_invalid = parts_dropped_cols.na.drop(
            how="any", subset=["values", "namespaceName", "tableName"]
        )
    ...
    def transform_databases(self, ms_dbs, ms_database_params):
        dbs_with_params = self.join_with_params(
            df=ms_dbs, df_params=ms_database_params, id_col="DB_ID"
        )
        ...
        dbs_dropped_cols = dbs_renamed.drop_columns(
            ["DB_ID", "OWNER_NAME", "OWNER_TYPE"]
        )
        dbs_drop_invalid = dbs_dropped_cols.na.drop(how="any", subset=["name"])
        dbs_final = dbs_drop_invalid.select(
            struct(dbs_dropped_cols.columns).alias("item")
        ).withColumn("type", lit("database"))
        return dbs_final

    def transform(self, hive_metastore):
        dbs_prefixed = HiveMetastoreTransformer.add_prefix_to_column(
            hive_metastore.ms_dbs, "NAME", self.db_prefix
        )
        ...
        databases = self.transform_databases(
            ms_dbs=dbs_prefixed, ms_database_params=hive_metastore.ms_database_params
        )

        db_tbl_joined = HiveMetastoreTransformer.join_dbs_tbls(
            ms_dbs=dbs_prefixed, ms_tbls=tbls_prefixed
        )
